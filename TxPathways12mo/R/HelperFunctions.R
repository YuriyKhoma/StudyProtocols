renderStudySpecificSql <- function(studyName, minCellCount, cdmSchema, resultsSchema, sourceName, dbms){
  ...
  } else if (studyName == "DMca"){
    TxList <- '21600712,21500148'
    DxList <- '201820'
    ExcludeDxList <- '444094,201254'#updated code to fix type I DM code; previously was 35506621
  } else if (studyName == "DPca"){
  ...
  translatedSql <- translateSql(renderedSql, targetDialect = dbms)$sql
  ...
  extractAndWriteToFile <- function(connection, tableName, resultsSchema, sourceName, studyName, dbms){
    parameterizedSql <- "SELECT * FROM @resultsSchema.@studyName_@sourceName_@tableName"
